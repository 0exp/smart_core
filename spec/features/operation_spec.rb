...
describe SmartCore::Operation do
  ...
  specify 'finalizer smoke test' do
    ...
    service = SmokeFinalizer.new(1, 2, 3, pek: nil, jek: '55', fek: 33)
    ...
    class SuperSmokeFinalizer < SmokeFinalizer
      param :cho, finalize: :finalize_cho
      option :pho, finalize: :finalize_pho, default: -> { 22 }

      def finalize_cho(value)
        value.to_s
      end

      def finalize_pho(value)
        value.to_s
      end
    end

    service = SuperSmokeFinalizer.new(5, 6, 7, :mega_blast, pek: 1, jek: 2, fek: 3)

    expect(service.kek).to eq('5')
    expect(service.a).to eq(6)
    expect(service.b).to eq(7)
    expect(service.pek).to eq('fek')
    expect(service.jek).to eq(2)
  ...
  describe 'invokation and results' do
    ...
    specify 'callback result' do
      class CallbackResultOperation < SmartCore::Operation
        def call
          Callback { 'it_works!' }
        end
      end

      result = CallbackResultOperation.call
